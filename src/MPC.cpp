 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      // Evaluate the CTE and EPSI using the waypoint polynomial at the current longetudinal position
      AD<double> f0 = coeffs[0] * pow(x0, 0) + coeffs[1] * pow(x0, 1) + coeffs[2] * pow(x0, 2) + coeffs[3] * pow(x0, 3);
      AD<double> psides0 = CppAD::atan(coeffs[1] * pow(x0, 0) + 2 * coeffs[2] * pow(x0, 1) + 3 * coeffs[3] * pow(x0, 2));

      // Evaluate the cost for the next state of the model
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // TODO: set the lower and upper bounds for limits
  ...
  std::vector<double> mpc_x, mpc_y, mpc_result;

  for (int i = 0; i < N; ++i) {
    mpc_x.push_back(solution.x[x_start + i]);
    mpc_y.push_back(solution.x[y_start + i]);
  }
  mpc_result.insert(mpc_result.end(), mpc_x.begin(), mpc_x.end());
  mpc_result.insert(mpc_result.end(), mpc_y.begin(), mpc_y.end());
  mpc_result.push_back(N);
